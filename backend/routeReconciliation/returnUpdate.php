...
isset($_POST['vehicle'])){
    ...
    if ($result->num_rows == 1) {
        ...
        if($delivery['updated_status'] == "scan_pending"){
            ...
            $response = array(
                'requestStatus' => 500,
                'message' => 'Update Failed: The delivery you are attempting to update has already been scanned and uploaded into the Document Management System (DMS). Please refresh the Route Reconciliation page to view the latest details.'
            );
            echo json_encode($response);
        }
